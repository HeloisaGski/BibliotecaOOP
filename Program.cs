Biblioteca biblioteca = new Biblioteca();
        ...
        do
        {
            ...
            Console.WriteLine("4. Atualizar informações de um livro");
            Console.WriteLine("5. Atualizar informações de um usuário");
            Console.WriteLine("6. Mostrar usuários cadastrados");
            ...
            switch (opcao)
            {
                ...
                case 2:
                    ...
                    if (!string.IsNullOrEmpty(tituloEmprestar))
                    {
                ...
                case 3:
                    ...
                    if (!string.IsNullOrEmpty(tituloDevolver))
                    {
                ...
                case 4:
                    Console.Write("Digite o título do livro que deseja atualizar: ");
                    string? tituloAtualizar = Console.ReadLine();

                    if (!string.IsNullOrEmpty(tituloAtualizar))
                    {
                        Console.Write("Novo Título: ");
                        string novoTitulo = Console.ReadLine() ?? "Título Padrão";

                        Console.Write("Novo Autor: ");
                        string novoAutor = Console.ReadLine() ?? "Autor Desconhecido";

                        Console.Write("Novo ISBN: ");
                        string novoIsbn = Console.ReadLine() ?? "ISBN Desconhecido";

                        Console.Write("Novo Gênero: ");
                        string novoGenero = Console.ReadLine() ?? "Gênero Desconhecido";

                        Console.Write("Nova Quantidade: ");
                        int novaQuantidade;
                        while (!int.TryParse(Console.ReadLine(), out novaQuantidade))
                        {
                            Console.Write("Entrada inválida. Por favor, digite um número para a nova quantidade: ");
                        }

                        biblioteca.AtualizarLivro(tituloAtualizar, novoTitulo, novoAutor, novoIsbn, novoGenero, novaQuantidade);
                    }
                    else
                    {
                        Console.WriteLine("Título do livro não pode ser nulo.");
                    }
                    break;
                ...
                case 5:
                    ...
                    if (int.TryParse(Console.ReadLine(), out int idUsuario))
                    {
                        ...
                        string novoNome = Console.ReadLine() ?? "Nome Desconhecido";

                        Console.Write("Novo Endereço: ");
                        string novoEndereco = Console.ReadLine() ?? "Endereço Desconhecido";

                        Console.Write("Novo Telefone: ");
                        string novoTelefone = Console.ReadLine() ?? "Telefone Desconhecido";

                        biblioteca.AtualizarUsuario(idUsuario, novoNome, novoEndereco, novoTelefone);
                    }
                    else
                    {
